...
def _build_queued_tasks_query(*, dialect_name: str, limit: int, executor_type: str = "direct"):
    stmt = select(ExecutionTaskRecord).where(
        ExecutionTaskRecord.status == "queued",
        ExecutionTaskRecord.executor_type == executor_type,
    )
    stmt = stmt.order_by(ExecutionTaskRecord.priority.desc(), ExecutionTaskRecord.id).limit(limit)
